...
import config
from dex import LOGGER, app, userbot
from dex.core.call import Mody
from dex.misc import sudo
from dex.plugins import ALL_MODULES
from dex.utils.database import get_banned_users, get_gbanned
from config import BANNED_USERS
from dex.plugins.play.azan import azkar
...
async def init():
    ...
    for all_module in ALL_MODULES:
        importlib.import_module("dex.plugins" + all_module)
    LOGGER("dex.plugins").info("Successfully Imported Modules...")
    await userbot.start()
    await Mody.start()
    try:
    ...
    await userbot.stop()
    await azkar()
    LOGGER("dex").info("Stopping Source mix Bot...")
